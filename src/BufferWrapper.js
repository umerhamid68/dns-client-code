...
// }
// export { BufferWrapper };
var BufferWrapper = /** @class */ (function () {
    function BufferWrapper(sizeOrBuffer, offset) {
    ...
    BufferWrapper.prototype.setOffset = function (value) {
        this.offset = value;
    };
    BufferWrapper.prototype.writeUInt16BE = function (value) {
        this.buffer.writeUInt16BE(value, this.offset);
        this.offset += 2;
    };
    BufferWrapper.prototype.readUInt16BE = function (offset) {
        var value = this.buffer.readUInt16BE(offset !== undefined ? offset : this.offset);
        if (offset === undefined) {
            this.offset += 2;
        }
        return value;
    ...
    BufferWrapper.prototype.writeString = function (value) {
        this.buffer.write(value, this.offset);
        this.offset += value.length;
    };
    BufferWrapper.prototype.readUInt8 = function (offset) {
        var value = this.buffer.readUInt8(offset !== undefined ? offset : this.offset);
        if (offset === undefined) {
            this.offset += 1;
        }
        return value;
    };
    BufferWrapper.prototype.readString = function (length) {
        var value = this.buffer.toString('utf8', this.offset, this.offset + length);
        this.offset += length;
        return value;
    };
    BufferWrapper.prototype.writeDomainName = function (domain) {
        var _this = this;
        var labels = domain.split('.');
        labels.forEach(function (label) {
            _this.writeUInt8(label.length);
            _this.writeString(label);
        });
        this.writeUInt8(0); // null byte at the end
    };
    BufferWrapper.prototype.readDomainName = function () {
        var labels = [];
        var jumped = false;
        var jumpOffset = 0;
        while (this.buffer[this.offset] !== 0) {
            var length_1 = this.readUInt8();
            // compression
            if ((length_1 & 0xc0) === 0xc0) {
                if (!jumped) {
                    jumpOffset = this.offset + 1;
                }
                this.offset = ((length_1 & 0x3f) << 8) | this.readUInt8();
                jumped = true;
                continue;
            }
            labels.push(this.readString(length_1));
        }
        if (!jumped) {
    ...
    BufferWrapper.prototype.readUInt32BE = function () {
        var value = this.buffer.readUInt32BE(this.offset);
        this.offset += 4;
        return value;
    };
    BufferWrapper.prototype.readSubarray = function (length) {
